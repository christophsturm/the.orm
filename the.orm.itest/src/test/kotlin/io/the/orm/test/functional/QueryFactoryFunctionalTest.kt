...
class QueryFactoryFunctionalTest {
    val context =
        describeOnAllDbs("support for querying data", DBS.databases, SCHEMA) { connectionProvider ->
            ...
            describe("query language") {
                describe("has a typesafe query api") {
                    ...
                    it("can query by condition with one and two parameters") {
                        val findByUserNameLikeAndBirthdayBetween =
                            repo.queryFactory.createQuery(
                                User::name.like(),
                                User::birthday.between()
                            )

                        val result: List<User> =
                            findByUserNameLikeAndBirthdayBetween
                                .with("%", Pair(LocalDate.of(2000, 4, 2), LocalDate.of(2000, 6, 2)))
                    ...
                    it("can query by sql") {
                        val findByUserNameLikeAndBirthdayBetween =
                            repo.queryFactory.createQuery(
                                "name like ? and birthday between ? and ?"
                            )
                    ...
                    it(
                        ...
                        expectThat(
                                findIdIn
                                    .with(arrayOf(usersPerMonth[4].id!!, usersPerMonth[5].id!!))
                                    .find(connectionProvider)
                            )
                            .containsExactlyInAnyOrder(usersPerMonth[4], usersPerMonth[5])
                    }
                ...
                test("can query null values") {
                    val coolUser = create(User(name = "coolUser", email = "email1", isCool = true))
                    val uncoolUser =
                        create(User(name = "uncoolUser", email = "email2", isCool = false))
                    val userOfUndefinedCoolness =
                        create(
                            User(name = "userOfUndefinedCoolness", email = "email3", isCool = null)
                        )
                    val findByCoolness = repo.queryFactory.createQuery(User::isCool.isEqualTo())
                    val findByNullCoolness = repo.queryFactory.createQuery(User::isCool.isNull())
                ...
                describe("findOrCreate") {
                    ...
                    val queryByName =
                        repo.repo.queryFactory.createQuery(Vegetable::name.isEqualTo())
                    it("finds an existing entity") {
                        expectThat(
                                queryByName.with("carrot").findOrCreate(connectionProvider) {
                    ...
                    it("creates a new entity if it does not yet exist") {
                        expectThat(
                            queryByName.with("tomato").findOrCreate(connectionProvider) {
                ...
                describe("createOrUpdate") {
                    ...
                    it("creates a new entity if it does not yet exist") {
                        expectThat(
                            queryByName
                                .with("tomato")
                                .createOrUpdate(connectionProvider, Vegetable(name = "tomato"))
                        ) {
                            get { id }.isNotNull()
                            get { name }.isEqualTo("tomato")
                        }
                    ...
                    it("updates the existing entity if it already exists") {
                        ...
                        expectThat(
                            queryByName
                                ...
                                .createOrUpdate(
                                    connectionProvider,
                                    Vegetable(null, "carrot", weight = 20.0)
                                )
                        ) {

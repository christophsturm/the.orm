...
@Serializable
data class SerializableUser(val id: PKType? = null, val name: String, val email: String?)
...
class RepoFunctionalTest {
    ...
    val context =
        describeOnAllDbs("the repository class", DBS.databases, SCHEMA) { connection ->
            context("with a user class") {
                ...
                context("loading data objects") {
                    ...
                    test("throws NotFoundException when id does not exist") {
                        expectCatching { repo.findById(connection, 1) }
                            .isFailure()
                        ...
                        val color =
                            connection.withConnection { connection ->
                                connection
                                    ...
                                    .map { row -> row.get("favorite_color", String::class.java)!! }
                                    .single()
                            }

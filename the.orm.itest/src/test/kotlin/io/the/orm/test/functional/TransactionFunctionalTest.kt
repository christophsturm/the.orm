...
class TransactionFunctionalTest {
    val context =
        describeOnAllDbs("Transaction handling", DBS.databases, USERS_SCHEMA) { connectionProvider
            ->
                    ...
                    val user =
                        outerRepo.transaction { transactionRepo ->
                            ...
                            expectThat(
                                    userNameLike
                                        .with("%")
                                        .findSingle(transactionRepo.connectionProvider)
                                )
                                .isEqualTo(user)
                            // but the outer connection does not see it
                            expectThat(userNameLike.with("%").find(connectionProvider).count())
                                .isEqualTo(0)
                            user
                        }
                    // now the outer connection sees them too
                    expectThat(userNameLike.with("%").findSingle(connectionProvider))
                        .isEqualTo(user)
                }
                        ...
                        connectionProvider.transaction { transactionConnectionProvider ->
                            val user =
                                repo.create(
                                    ...
                                    User(name = "a user", email = "with email")
                                )
                            // the created user is visible in the same connection
                            expectThat(

        ...
        describeOnAllDbs(
            ...
            it("can batch insert values and select result") {
                val dbConnectionFactory =
                    (connection as TransactionalConnectionProvider).dbConnectionFactory
                ...
                val (firstId, secondId) =
                    conn
                        .createStatement("insert into users(name) values($1)")
                        .bind("$1", "belle")
                        .add()
                        .bind("$1", "sebastian")
                        ...
                        .asFlow()
                        .map {
                            it.map { row, _ -> row.get(0, Long::class.java)!!.toLong() }
                                .awaitSingle()
                        }
                        .toList()
                val selectResult =
                ...
                val namesFlow =
                    selectResult
                        .map { row, _ -> row.get("NAME", String::class.java) as String }
                        .asFlow()
                        ...
                        test("can open and close pool") {
                            ...
                            val factory =
                                ConnectionFactories.get(
                                    "r2dbc:pool:postgresql://test:test@$host:$port/$databaseName?initialSize=1"
                                )

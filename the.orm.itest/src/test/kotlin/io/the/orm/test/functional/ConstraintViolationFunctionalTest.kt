...
class ConstraintViolationFunctionalTest {
    val context =
        describeOnAllDbs("constraint error handling", DBS.databases, USERS_SCHEMA) {
            connectionProvider ->
            ...
            it("throws DataIntegrityViolationException exception on constraint violation") {
                val user =
                    User(name = "chris", email = "email", birthday = LocalDate.parse("2020-06-20"))
                repo.create(user)

                expectThrows<UniqueConstraintViolatedException> { repo.create(user) }
                    .and {

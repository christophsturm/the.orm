...
class VertxTest {
    ...
    val context = describe(
        ...
        it("can run sql queries") {
            val result: RowSet<Row> = client.query("SELECT * FROM users WHERE id=1").execute().coAwait()
            expectThat(result.size()).isEqualTo(0)
        }
        it("can run prepared queries") {
            val result: RowSet<Row> =
                client.preparedQuery("SELECT * FROM users WHERE id=$1").execute(Tuple.of(1)).coAwait()
            expectThat(result.size()).isEqualTo(0)
        }
        ...
        describe("querying by lists") {
            ...
            it("works with one parameter per item") {
                ...
                assert(query.execute(Tuple.from(listOf(1, 2))).coAwait().size() == 2)
            }
            it("works with ANY") {
                val query = client.preparedQuery("SELECT * FROM users WHERE id = ANY($1)")
                assert(query.execute(Tuple.of(arrayOf(1, 2))).coAwait().size() == 2)
            }

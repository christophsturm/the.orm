...
class DBConnectionTest {
    val context =
        describeOnAllDbs<DBConnection>(DBS.databases, SCHEMA) { connectionProvider ->
            describe("inserting with autoincrement") {
                it("works when all fields are non-null") {
                    val result =
                        connectionProvider.withConnection { connection ->
                            connection
                                .createInsertStatement("insert into users(name) values ($1)")
                                .execute(listOf(String::class.java), listOf("belle"))
                                .getId()
                        }
                        ...
                        connectionProvider.withConnection { connection ->
                            connection
                                ...
                                .executeBatch(
                                    listOf(String::class.java, String::class.java),
                                    listOf(
                                        listOf("belle", "belle@bs.com"),
                                        listOf("sebastian", "seb@bs.com")
                                    )
                                )
                                .map { it.getId() }
                                .toList()
            ...
            describe("selecting") {
                ...
                connectionProvider.withConnection { connection ->
                    ...
                    repeat(2) {
                        connection
                            .createInsertStatement("insert into users(name, bio) values ($1,$2)")
                            .execute(listOf(String::class.java), listOf("belle", bio))
                            .getId()
                    }
                }
                it("returns query results as flow of maps") {
                    val result =
                        connectionProvider.withConnection {
                            it.createStatement("select id, name, email, bio from users")
                                .execute()
                                .asMapFlow()
                                .toList()
                        }
                    ...
                    assert(
                        result[1] ==
                            mapOf("id" to 2L, "name" to "belle", "email" to null, "bio" to bio)
                    )
                }

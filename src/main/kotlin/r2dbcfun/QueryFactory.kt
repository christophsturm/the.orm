...
public class QueryFactory<T : Any> internal constructor(kClass: KClass<T>, private val finder: Finder<T>) {
    ...
    public fun <P1 : Any, P2 : Any> createQuery(p1: Condition<P1>, p2: Condition<P2>): TwoParameterQuery<P1, P2> =
    ...
    public inner class Query internal constructor(
        ...
        public suspend fun find(connection: Connection, vararg parameter: Any): Flow<T> {
            val parameterValues = parameter.asSequence()
                .flatMap { if (it is Pair<*, *>) sequenceOf(it.first as Any, it.second as Any) else sequenceOf(it) }

...
private fun <T, V> KProperty1<T, V>.like(v: V): Condition<T, V> =
    Condition(this, "like(?)", listOf(v))
...
class Query<T : Any>(kClass: KClass<T>, conditions: List<Condition<T, *>>) {
    ...
    internal val queryString =
        conditions.joinToString(separator = " and ") {
            "${snakeCaseForProperty[it.property]} ${it.queryFragment}"
        }
